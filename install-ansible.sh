...
if [ $? -eq 1 ]; then
  ...
  if [ -z $ANSIBLE_DEBUG ]; then
    apt-get -qq --force-yes --assume-yes update > /dev/null 2>&1
    apt-get -qq --force-yes --assume-yes install git python-cryptography python-setuptools python-dev python-paramiko python-yaml python-jinja2 python-httplib2 python-passlib python-six python-ecdsa > /dev/null 2>&1
  else
    apt-get --assume-yes update
    apt-get --assume-yes install git python-cryptography python-setuptools python-dev python-paramiko python-yaml python-jinja2 python-httplib2 python-passlib python-six python-ecdsa
  fi
